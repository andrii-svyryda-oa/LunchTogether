...
- [Додаток Г. Зведення ендпоінтів REST API](#додаток-г-зведення-ендпоінтів-rest-api)
...
**Таблиця 1.2.1. Скорочений порівняльний аналіз існуючих рішень**
...
| Самостійне розгортання без зовнішнього сервісу | Ні | Ні | Так | Ні | Так |
...
**Таблиця 1.2.2. Узагальнений SWOT-аналіз системи «LunchTogether»**
...
**Таблиця 1.5.1. Узагальнення функціональних вимог за модулями**
...
Деталізований перелік з 37 вимог використовується у матриці трасування розділу [6.6](#66-матриця-трасування-вимог); у цьому розділі залишено лише рівень, потрібний для постановки задачі.
...
**Таблиця 6.4.1. Зведення інтеграційних тестів**

| Файл тестів | Основна область перевірки | Приклади критичних сценаріїв |
| ----------- | ------------------------- | ---------------------------- |
| `test_auth.py` | Автентифікація | Реєстрація, логін, JWT-cookie, `/me`, logout |
| `test_users.py` | Користувачі | Адмінські дії, власний профіль, захист від самодеактивації |
| `test_groups.py` | Групи | Створення, ліміти, видимість груп, видалення |
| `test_invitations.py` | Запрошення | Токени, email-відправка, accept/decline, ліміт 25 учасників |
| `test_permissions.py` | Дозволи | Пресети ролей, точкова зміна дозволів, заборонені дії |
| `test_restaurants.py` | Ресторани | CRUD ресторанів і страв, улюблені позиції |
...
**Таблиця 6.6.1. Скорочена матриця трасування вимог**
...
У межах дипломної роботи розроблено веб-додаток «LunchTogether» для організації спільних обідів у колективах. Робота охопила аналіз предметної області, постановку вимог, архітектурне проєктування, реалізацію серверної та клієнтської частин, розгортання на VPS і програму тестування.
...
## Додаток Г. Зведення ендпоінтів REST API

Повний перелік маршрутів реалізовано у директорії [backend/app/api/](../backend/app/api/). У тексті додатку залишено згрупований вигляд, достатній для розуміння структури API; детальний контракт доступний через OpenAPI-документацію FastAPI у середовищі розробки.

**Таблиця Г.1. Групи REST-ендпоінтів**
...
| Invitations | `/api/groups/.../invitations` | створення, список, accept, decline, cancel | AUTH / EDITOR(members) |
