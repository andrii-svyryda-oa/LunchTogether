...
class UpdateOrderItemInput(BaseModel):
...
class UpdateOrderItemOutput(BaseModel):
    item: OrderItemResponse


class UpdateOrderItemWorkflow:
    def __init__(
        self,
    ...
    async def execute(self, input_data: UpdateOrderItemInput) -> UpdateOrderItemOutput:
        ...
        if not input_data.data.model_dump(exclude_unset=True):
            return UpdateOrderItemOutput(
                item=OrderItemResponse(
                    **{k: getattr(item, k) for k in OrderItemResponse.model_fields if hasattr(item, k)},
                )
            )

        updated = await self.order_item_repository.update(input_data.item_id, input_data.data)
        return UpdateOrderItemOutput(
            item=OrderItemResponse(
