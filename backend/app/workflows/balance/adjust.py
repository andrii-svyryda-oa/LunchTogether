...
from app.schemas.balance import BalanceAdjustmentCreate, BalanceResponse
from app.schemas.internal import BalanceAmountUpdate, BalanceHistoryInternalCreate, BalanceInternalCreate
...
class AdjustBalanceWorkflow:
    ...
    async def execute(self, input_data: AdjustBalanceInput) -> AdjustBalanceOutput:
        ...
        balance = await self.balance_repository.get_by_user_and_group(input_data.data.user_id, input_data.group_id)
        if balance is None:
            balance = await self.balance_repository.create(
                BalanceInternalCreate(user_id=input_data.data.user_id, group_id=input_data.group_id)
            )
